...
export class HealthSystem {
  ...
  effList() {
    ...
    for (const k in this.eff) {
      ...
      a.push(
        (kind === 'light' ? 'Лёгкое кровотечение' : kind === 'heavy' ? 'Сильное кровотечение' : kind === 'fracture' ? 'Перелом' : kind) +
          ' — ' +
          (p === 'head' ? 'Голова' : p === 'thorax' ? 'Грудная клетка' : p === 'stomach' ? 'Живот' : p === 'larm' ? 'Левая рука' : p === 'rarm' ? 'Правая рука' : p === 'lleg' ? 'Левая нога' : 'Правая нога')
      );
    }
  ...
  fixedUpdate(h, ctx = this.ctx) {
    ...
    if (this.energy <= 0 || this.hydration <= 0) {
      ...
      for (let i = 0; i < PARTS.length; i++) if (this.hp[i] > 0 && this.hp[i] < this.max[i]) this.hp[i] = Math.min(this.max[i], this.hp[i] + 0.12 * dt);
    }
  }
