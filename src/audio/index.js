...
export class AudioSystem {
  ...
  /** 
   * Окклюзия через физику, интегрированная в архитектуру Claude-of-Duty.
   * Адаптировано под вызовы пространственных звуков через SpatialField.
   */
  playProcedural(kind, position, opts = {}) {
    if (!this.running || this.actx.state === 'suspended') return false;
    if (!position || !isVec(position)) return this.play(kind, position, opts);

    const phys = this.ctx.peek('physics');
    const lp = this.field.listenerPos;
    
    // Проверяем прямую видимость через физический движок (Raycast / LineOfSight)
    let blocked = false;
    if (phys?.lineOfSight) {
      blocked = !phys.lineOfSight(position, lp, phys.MASK?.WORLD);
    }
    
    // Настраиваем глухой застенный звук (lowpass фильтр): 900 Гц при окклюзии
    const finalOpts = {
      ...opts,
      occlusion: blocked ? 1 : 0,
      frequency: blocked ? 900 : 18000
    };

    const k = UI_ALIAS[kind] ?? kind;
    return this._playAt(k, position.x, position.y, position.z, finalOpts,
      opts.bus ?? BUS_FOR[k] ?? 'foley', opts.priority ?? 0.5);
  }
}